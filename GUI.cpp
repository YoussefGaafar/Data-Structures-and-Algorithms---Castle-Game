...
void GUI::UpdateStatusBar(int CurrentTimeStep,  double health, CASTLE_STATUS stat, int actfi, int actfre, int acthea, int acttot, int frofi, int frofre, int froheal, int frotot, int killfi, int killfre, int killheal, int totkill)
{
		...
		double d=1;
		pWind->DrawString(10, WindHeight - (int)(StatusBarHeight / d),"Health of the castle ="+ to_string((int)health));
		d += 0.15;
		if (stat == ACTIV)
		{
			pWind->DrawString(10, WindHeight - (int)(StatusBarHeight / d), "Status of the castle is Active");
		}
		else if (stat == FROST)
		{
			pWind->DrawString(10, WindHeight - (int)(StatusBarHeight / d), "Status of the castle is Frosted");
		}
		else
			pWind->DrawString(10, WindHeight - (int)(StatusBarHeight / d), "Status of the castle is killed");


		pWind->DrawString(10,560, "no of active fighters =" + to_string((int)actfi));

		pWind->DrawString(10,575, "no of active frezzers= " + to_string((int)actfre));
	
		pWind->DrawString(10, 590, "no of active healers= " + to_string((int)acthea));
		
		pWind->DrawString(10, 615, "Total no of active enemies= " + to_string((int)acttot));

		pWind->DrawString(250, 560, "no of frosted fighters =" + to_string((int)frofi));

		pWind->DrawString(250, 575, "no of frosted frezzers= " + to_string((int)frofre));

		pWind->DrawString(250, 590, "no of frosted healers= " + to_string((int)froheal));

		pWind->DrawString(250, 615, "Total no of frosted enemies= " + to_string((int)frotot));

		pWind->DrawString(490, 560, "no of killed fighters =" + to_string((int)killfi));

		pWind->DrawString(490, 575, "no of killed frezzers= " + to_string((int)killfre));

		pWind->DrawString(490, 590, "no of killed healers= " + to_string((int)killheal));

		pWind->DrawString(490, 615, "Total no of killed enemies= " + to_string((int)totkill));

		//itoa(health, strTimestep, 10);
		//PrintMessage(strTimestep);
}

void GUI::UpdateInterface(int CurrentTimeStep, double health, CASTLE_STATUS stat, int actfi, int actfre, int acthea, int acttot, int frofi, int frofre, int froheal, int frotot, int killfi, int killfre, int killheal, int totkill)
{
	ClearDrawingArea();
	UpdateStatusBar(CurrentTimeStep, health,  stat,  actfi,  actfre,  acthea,  acttot,  frofi,  frofre,  froheal,  frotot,  killfi,  killfre,  killheal,  totkill);
...
void GUI::AddToDrawingList(const Enemy* pE)
{
	...
	const fighter* P = dynamic_cast<const fighter*>(pE);
	if (P)
	{
		 eType = 0;
	}
	...
	{
		const Freezer* Fr = dynamic_cast<const Freezer*>(pE);
			if (Fr)
				eType = 2;
			else 
				eType = 1;
...
PROG_MODE	GUI::getGUIMode() const
{
	...
	do
	{
		PrintMessage("Please select GUI mode:(For Phase 1 please select interactive) (1)Interactive, (2)StepByStep, (3)Silent,");
		string S = GetString();
